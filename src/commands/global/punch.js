...
module.exports = {
    ...
    async execute(message, args, client) {
        try {
            ...
            if (!user) {
                const errorMsg = await message.reply(errores.FALTA_USUARIO(`${config.prefix}punch @usuario`));
            ...
            if (user.id === message.author.id) {
                const errorMsg = await message.reply('❌ No puedes darte un puñetazo a ti mismo.');
                setTimeout(() => errorMsg.delete().catch(console.error), errores.TIEMPO_BORRADO_ERROR);
                setTimeout(() => message.delete().catch(console.error), errores.TIEMPO_BORRADO_ERROR);
                return;
            }
            ...
            const punchEmbed = new EmbedBuilder()
                .setColor('#FF69B4')
                .setTitle('🥊 ¡Puñetazo! 🥊')
                .setDescription(`**${message.author.username}** le dio un puñetazo a **${user.username}** 🥊`)
                .setImage(gifUrl)
                .setFooter({ text: `Anime: ${animeName}` })
                .setTimestamp();
            await message.reply({ embeds: [punchEmbed] });
        } catch (error) {
            console.error('Error en el comando punch:', error);
